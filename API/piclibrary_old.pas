...
var  SPBRG : Double;
Scale : Word;
Function Division(DivC:Integer):Double; //inline;
  begin
    Result := Clock*1000*1000/(Scale*DivC)-1;
  end;
    ...
    begin
      ...
      case Cpu of
        PIC18F4520:
          begin
            Scale := 16;
            SPBRG := Division(BaudRate);
            if SPBRG>63 then  WriteMemo('Установлен медленный режим порта')
            else
              begin
                Scale := 4;
                WriteMemo('Установлен быстрый режим порта '+InttoStr(N));
                SPBRG := Division(BaudRate);
              end;
            if SPBRG>255 then SPBRG := 255;
            WriteMemo('Установлена скорость '+IntToStr(Round(Division(Round(SPBRG)))));
          end;
      end;
